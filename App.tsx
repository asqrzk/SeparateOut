...
import { AppStage, Language, LinkedInDraft, PostPoint, Tone } from './types';
import { appCopy, toneOptions } from './i18n/copy';
import { 
...
const App: React.FC = () => {
  ...
  const [language, setLanguage] = useState<Language>(() => (
    localStorage.getItem('app_lang') === 'ar' ? 'ar' : 'en'
  ));
  const showOverlay = showIntro || showKeyModal;
  const t = appCopy[language];
  const slideLabel = (n: number) => (language === 'ar' ? `الشريحة ${n}` : `Slide ${n}`);
  const slidePlaceholder = (n: number) => (
    language === 'ar'
      ? `صف الفكرة للشريحة رقم ${n}...`
      : `Describe insight for slide #${n}...`
  );
  ...
  const handleApiError = async (err: any) => {
    ...
    if (err?.message?.includes("API Key not found") || err?.message?.includes("API_KEY_INVALID")) {
      setHasApiKey(false);
      setShowKeyModal(true);
      setError(t.errors.apiKey);
      setShowBillingGuide(false);
    } else if (err?.message?.includes("Requested entity was not found.")) {
      setError(t.errors.paidKey);
  ...
  const handleCreateDraft = async () => {
    ...
    if (!topic.trim() || activePoints.length === 0) {
      setError(t.errors.missingTopic);
      setShowBillingGuide(false);
      return;
    }
    setIsLoading(true);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors">
      {showIntro && <IntroScreen onComplete={handleStartApp} language={language} />}
      {showKeyModal && <ApiKeyModal onSuccess={handleKeySuccess} language={language} />}
      {!showOverlay && (
        <>
            ...
            <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                {!hasApiKey && (
                  ...
                  >
                    <AlertCircle className="w-4 h-4" /> {t.header.setApiKey}
                  </button>
                )}
                <button
                  onClick={() => setLanguage(language === 'en' ? 'ar' : 'en')}
                  className="px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors shadow-sm"
          ...
          <main className="max-w-6xl mx-auto px-6 py-12">
            {isLoading ? (
              <Loading status={loadingMsg} step={2} language={language} />
            ) : (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                {stage === 'INPUT' && (
                  <div className="max-w-4xl mx-auto space-y-12">
                    <div className="text-center space-y-4">
                      ...
                      <p className="text-slate-500 dark:text-slate-400 text-lg font-medium">{t.input.tagline}</p>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 sm:p-10 border border-slate-200 dark:border-white/10 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.14)] space-y-12">
                      ...
                      {error && (
                        <div className="p-5 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/30 rounded-2xl flex items-start gap-4 text-red-600 dark:text-red-400 text-sm font-medium animate-in shake duration-300">
                           ...
                           <div className="space-y-1">
                             <p className="font-bold">{t.input.errorTitle}</p>
                             <p className="text-xs opacity-80">{error}</p>
                             {showBillingGuide && (
                               <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="text-xs underline font-bold hover:text-red-700 transition-colors">{t.input.billingGuide}</a>
                             )}
                           </div>
                        </div>
                      )}
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center gap-3">
                           ...
                           <h3 className="text-xl font-bold">{t.input.step1}</h3>
                        </div>
                      ...
                      <div className="space-y-8">
                        <div className="flex items-center gap-3">
                           ...
                           <h3 className="text-xl font-bold">{t.input.step2}</h3>
                        </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {pointInputs.map((text, idx) => (
                              <div key={idx} className="relative group">
                                <div className="absolute left-5 top-5 text-[10px] font-black text-blue-500/20 uppercase tracking-[0.3em] z-10">{slideLabel(idx + 1)}</div>
                                <textarea 
                                  ...
                                  placeholder={slidePlaceholder(idx + 1)}
                                  value={text}
                                  onChange={(e) => {
                      ...
                      >
                        {t.input.generate} <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                      </button>
                    </div>
                  </div>
                </div>
                ...
                {stage === 'EDITING' && draft && (
                  <div className="max-w-4xl mx-auto space-y-12 pb-24">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                      <div className="space-y-1">
                        <button onClick={() => setStage('INPUT')} className="flex items-center gap-2 text-slate-400 hover:text-blue-600 transition-colors font-black text-[10px] uppercase tracking-widest">
                          <ArrowLeft className="w-4 h-4" /> {t.editing.startNew}
                        </button>
                        <h2 className="text-4xl font-black tracking-tight">{t.editing.postPreview}</h2>
                      </div>
                      <div className="flex flex-wrap gap-3">
                        ...
                        <button onClick={copyToClipboard} className="flex items-center justify-center gap-2 px-8 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-500 font-bold shadow-2xl transition-all active:scale-95 text-xs">
                          ...
                          {copied ? t.editing.copied : t.editing.copyPostText}
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="pt-20">
                       <SearchResults results={draft.searchResults} searchEntryPointHtml={draft.searchEntryPointHtml} language={language} />
                    </div>
                  </div>
                )}
